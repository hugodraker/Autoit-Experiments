...
; GLOBALS
; ============================================================
Global $g_aValues[0]          ; numeric values
Global $g_aLabels[0]          ; text labels (for pie legend)
...
; GUI
; ============================================================
$hGUI = GUICreate("CSV Chart Viewer (GDI)", 900, 600)
...
Func _DrawChartGDI($hDC, $w, $h)
    ...
    If $g_sChartType <> "pie" Then
        ...
        _DrawAxisNumericLabelsGDI($hDC, $left, $top, $plotW, $plotH)
        _DrawAxisTextLabelsGDI($hDC, $left, $top, $plotW, $plotH)
    EndIf

    Switch $g_sChartType
        Case "line"
            _DrawLineChartGDI($hDC, $left, $top, $plotW, $plotH)
            If $g_bTrend Then _DrawTrendLineGDI($hDC, $left, $top, $plotW, $plotH)
        Case "bar"
            _DrawBarChartGDI($hDC, $left, $top, $plotW, $plotH)
            If $g_bTrend Then _DrawTrendLineGDI($hDC, $left, $top, $plotW, $plotH)
        Case "pie"
...
Func _DrawAxisNumericLabelsGDI($hDC, $left, $top, $plotW, $plotH)
    Local $hFont = _WinAPI_CreateFont(14, 0, 0, 0, 400, False, False, False, $DEFAULT_CHARSET, _
    ...
    ; Y-axis numeric labels 0..100
    Local $rows = 10
    For $i = 0 To $rows
        Local $value = 100 - (10 * $i)
        Local $y = $top + ($plotH * $i) / $rows
        Local $text = String($value)
        _WinAPI_TextOut($hDC, 10, $y - 7, $text)
    Next

    ; X-axis numeric labels (index)
    Local $count = UBound($g_aValues)
    If $count > 0 Then
        Local $cols = $count
        If $cols > 20 Then $cols = 20
        For $i = 0 To $cols - 1
            Local $x = $left + ($plotW * ($i + 0.5)) / $cols
            Local $text = String($i + 1)
            _WinAPI_TextOut($hDC, $x - 5, $top + $plotH + 5, $text)
        Next
    EndIf
...
Func _DrawAxisTextLabelsGDI($hDC, $left, $top, $plotW, $plotH)
    ...
    ; Y-axis label (simple, not rotated)
...
Func _DrawLineChartGDI($hDC, $left, $top, $plotW, $plotH)
    Local $count = UBound($g_aValues)
    If $count < 2 Then Return

    Local $hPen = _WinAPI_CreatePen($PS_SOLID, 2, 0x0000FF) ; red (BGR)
    Local $hOldPen = _WinAPI_SelectObject($hDC, $hPen)

    For $i = 1 To $count - 1
        Local $x1 = $left + ($plotW * ($i - 1)) / ($count - 1)
        Local $y1 = _ScaleValueToY($g_aValues[$i - 1], $top, $plotH)
        Local $x2 = $left + ($plotW * $i) / ($count - 1)
        Local $y2 = _ScaleValueToY($g_aValues[$i], $top, $plotH)
...
Func _DrawBarChartGDI($hDC, $left, $top, $plotW, $plotH)
    Local $count = UBound($g_aValues)
    ...
    For $i = 0 To $count - 1
        ...
        Local $yTop = _ScaleValueToY($g_aValues[$i], $top, $plotH)
...
Func _DrawTrendLineGDI($hDC, $left, $top, $plotW, $plotH)
    Local $n = UBound($g_aValues)
    If $n < 2 Then Return
    ...
    Local $hPen = _WinAPI_CreatePen($PS_SOLID, 2, 0x0000FF) ; red trend line
    Local $hOldPen = _WinAPI_SelectObject($hDC, $hPen)
    ...
    Local $px0 = $left
    Local $py0 = _ScaleValueToY($y0, $top, $plotH)
    Local $px1 = $left + $plotW
    Local $py1 = _ScaleValueToY($y1, $top, $plotH)
...
Func _DrawPieChartGDI($hDC, $left, $top, $plotW, $plotH, $w, $h)
    ...
    For $i = 0 To $n - 1
        ...
        _WinAPI_Pie($hDC, $cx, $cy, $cx + $diam, $cy + $diam, _
            _PiePointX($cx, $diam, $start), _PiePointY($cy, $diam, $start), _
            _PiePointX($cx, $diam, $start + $sweep), _PiePointY($cy, $diam, $start + $sweep))
...
Func _SaveMetaFile($bEMF)
    ...
    Local $hMetaDC = _WinAPI_CreateEnhMetaFile($hDCRef, $sFile, 0, "CSV Chart Viewer")
