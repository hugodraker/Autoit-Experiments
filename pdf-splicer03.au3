...
Global $sRawContent = "" ; Keeps track of raw file data for reconstruction

; Create Main GUI (Resizable)
Global $hMainGUI = GUICreate("Native PDF Page Reader & Editor", 500, 400, -1, -1, BitOR($WS_MINIMIZEBOX, $WS_SIZEBOX, $WS_THICKFRAME, $WS_MAXIMIZEBOX))
...
; Create Controls (ListBox on left, Management layout on right)
    ...
    Switch $nMsg
        ...
        Case $mOpen
            ...
            If Not @error Then
                ...
                _ParsePDF($sFilePath)
                _PopulateListBox()
                WinSetTitle($hMainGUI, "", "Native PDF Reader - " & $sFilePath)
            EndIf
        ...
        Case $mManual
            MsgBox($MB_ICONINFORMATION, "User Manual", "1. Open a PDF." & @CRLF & "2. Select target items to remove." & @CRLF & "3. Click Delete Page(s) and use File -> Save to compile structural modifications.")
            
        Case $mAbout
            MsgBox($MB_ICONINFORMATION, "About", "Native Page Level Object Manipulator via AutoIt.")
    EndSwitch
WEnd
...
Func _DeleteSelectedPages()
    ...
    Local $iConfirm = MsgBox(BitOR($MB_YESNO, $MB_ICONQUESTION), "Confirm Deletion", "Remove " & $iSelCount & " page(s)? (Changes apply upon saving.)")
...
Func _ShowPropertiesWindow()
    ...
    Local $idSaveBtn = GUICtrlCreateButton("&Save Changes", 160, 325, 120, 30)
...
Func _WritePDFProperties($sTargetFile)
    Local $sWorkingContent = $sRawContent
    Local $iCurrentBoxCount = _GUICtrlListBox_GetCount($idListBox)
    
    ; Structural Page Array Modification Engine
    If $iCurrentBoxCount < $iPageCount Then
        Local $aKidsMatch = StringRegExp($sWorkingContent, "(?i)/Kids\s*\[([^\]]*)\]", 3)
        If Not @error Then
            Local $sOriginalKids = $aKidsMatch[0]
            Local $aIndividualKids = StringRegExp($sOriginalKids, "(\d+\s+\d+\s+R)", 3)
            
            If Not @error And UBound($aIndividualKids) = $iPageCount Then
                Local $sNewKids = ""
                ; Rebuild the /Kids reference string using the elements left in the UI listbox
                For $idx = 1 To $iPageCount
                    Local $iFind = _GUICtrlListBox_FindString($idListBox, "Page " & $idx)
                    If $iFind <> -1 Then
                        $sNewKids &= $aIndividualKids[$idx - 1] & " "
                    EndIf
                Next ; <-- Fixed structural typo here
                
                $sWorkingContent = StringReplace($sWorkingContent, $sOriginalKids, StringStripWS($sNewKids, 3))
                $sWorkingContent = StringRegExpReplace($sWorkingContent, "(?i)/Count\s+\d+", "/Count " & $iCurrentBoxCount)
                $iPageCount = $iCurrentBoxCount
            EndIf
        EndIf
    EndIf

    ; Metadata Dictionary Injection Engine
    ...
    If $sAuthor <> "" Then $sNewInfoObj &= "/Author (" & $sAuthor & ") "
    If $sKeywords <> "" Then $sNewInfoObj &= "/Keywords (" & $sKeywords & ") "
    $sNewInfoObj &= ">> >>" & @CRLF
    
    Local $iEofPos = StringInStr($sWorkingContent, "%%EOF", 0, -1)
    If $iEofPos > 0 Then
        $sWorkingContent = StringLeft($sWorkingContent, $iEofPos - 1) & $sNewInfoObj & "%%EOF"
    Else
        $sWorkingContent &= $sNewInfoObj & "%%EOF"
    EndIf
    
    Local $hOut = FileOpen($sTargetFile, BitOR($FO_OVERWRITE, $FO_BINARY))
    If $hOut <> -1 Then
        FileWrite($hOut, StringToBinary($sWorkingContent, 1))
        FileClose($hOut)
        $sRawContent = $sWorkingContent 
        MsgBox($MB_ICONINFORMATION, "Success", "PDF updated and written successfully.")
    EndIf
EndFunc