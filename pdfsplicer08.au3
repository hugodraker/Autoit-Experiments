...
; DELETE SELECTED PAGES FROM LISTBOX
Func _DeleteSelectedPages()
    Local $aSel = _GUICtrlListBox_GetSelItems($lbPages)
    If Not IsArray($aSel) Then Return

    If MsgBox($MB_YESNO + $MB_ICONWARNING, "Confirm Delete", "Are you sure you want to delete the selected pages?") = $IDNO Then Return

    ; Delete from bottom up
    For $i = UBound($aSel) - 1 To 0 Step -1
        _GUICtrlListBox_DeleteString($lbPages, $aSel[$i])
    Next
EndFunc
