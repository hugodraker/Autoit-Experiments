...
; ============================================================
; ASCII85 DECODE (for /ASCII85Decode)
; ============================================================
Func _ASCII85Decode($s)
    Local $out = Binary("0x")
    Local $len = StringLen($s)
    Local $i = 1, $group[5], $g = 0
    ...
    While $i <= $len
        ...
        If $g = 5 Then
            ...
            For $k = 0 To 4
                $val = $val * 85 + $group[$k]
            Next
            Local $chunk = Binary("0x" & Hex(BitShift($val, -24) And 0xFF, 2) & _
    ...
    If $g > 0 Then
        ...
        Local $chunk = Binary("0x" & Hex(BitShift($val, -24) And 0xFF, 2) & _
                                    ...
                                    Hex(BitShift($val, -8)  And 0xFF, 2) & _
                                    Hex($val And 0xFF, 2))
        ; only first g-1 bytes are valid
...
; ============================================================
; APPLY FILTER CHAIN (ASCII85 + FlateDecode)
; ============================================================
Func _ApplyFilters($filters, $streamData)
    ...
    For $i = 0 To UBound($filters) - 1
        Switch StringLower($filters[$i])
            ...
            ; other filters could be added here (LZWDecode, etc.)
        EndSwitch
    Next
...
; ============================================================
; PDF STREAM DECODER → RAW BINARY STREAMS (more compatible)
; ============================================================
Func _PDF_DecodeRawStreams($in, $out)
    ...
    For $i = UBound($objs) - 1 To 0 Step -1
        ...
        _Log("")
        _Log("Object #" & ($i + 1) & " has filters: " & StringJoin($filters, ", "))
        ...
        If @error Or Not IsBinary($bDecomp) Then
            _Log("  ERROR: Filter chain failed.")
            ContinueLoop
        EndIf
...
Func _Main()
    ...
    $g_hGUI = GUICreate("PDF Raw Stream Decoder (multi-filter, zlib udf.au3)", 900, 550)
