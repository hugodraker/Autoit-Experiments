...
GUICtrlSetBkColor($btnSearch, 0x00AA00) ; green when idle

$lblStatus = GUICtrlCreateLabel("Status: Idle", 10, 70, 400, 20)

$lvResults = GUICtrlCreateListView("Line|File Name|Directory", 10, 100, 870, 530, _
    BitOR($LVS_REPORT, $LVS_SHOWSELALWAYS))
_GUICtrlListView_SetColumnWidth($lvResults, 0, 60)
_GUICtrlListView_SetColumnWidth($lvResults, 1, 200)
_GUICtrlListView_SetColumnWidth($lvResults, 2, 600)
...
While True
    ...
    Switch $msg
        ...
        Case $btnSearch
            If GUICtrlRead($btnSearch) = "Search" Then
                _StartSearch()
            Else
                $g_bStop = True
            EndIf
...
Func _StartSearch()
    ...
    GUICtrlSetBkColor($btnSearch, 0xAA0000) ; red
    ...
    For $i = 1 To 4000
        ...
        If StringRegExp($sLine, "^\d{2}/\d{2}/\d{4}") Then
            ...
            If StringInStr($sFileName, $g_sTerm, 0) > 0 Then
                ...
                GUICtrlCreateListViewItem($g_iLine & "|" & $sFileName & "|" & $g_sCurrentDir, $lvResults)
            EndIf
        EndIf
...
; -----------------------------
; HANDLE LIST CLICK (double/right click)
; -----------------------------
...
Func _IsRightClick()
    Return BitAND(GUICtrlGetState($lvResults), $GUI_EVENT_SECONDARYDOWN)
EndFunc

Func _IsDoubleClick()
    Return BitAND(GUICtrlGetState($lvResults), $GUI_EVENT_DBLCLICK)
EndFunc
...
Func _EndSearch($sMsg)
    ...
    GUICtrlSetBkColor($btnSearch, 0x00AA00) ; green
EndFunc
