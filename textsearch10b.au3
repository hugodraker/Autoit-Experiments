...
While True
    ...
    Switch $msg
        ...
        Case $btnBrowse
            ; Right-click → open index file in Notepad
            If _IsPressed("02") Then
                Local $idxFile = GUICtrlRead($inpFile)
                If FileExists($idxFile) Then ShellExecute("notepad.exe", '"' & $idxFile & '"')
            Else
...
; -----------------------------
; INDEX FOLDER (native, real dates)
; -----------------------------
Func _IndexFolder()
    ...
    For $i = 1 To $aFiles[0]
        ...
        Local $t = FileGetTime($full, 0, 1)
        If Not IsArray($t) Then ContinueLoop

        Local $dt = _FormatDirDateTime($t)
        FileWriteLine($hOut, $dt & " " & $file)
    Next
...
; Format timestamp like DIR: MM/DD/YYYY  HH:MM AM/PM + size column
Func _FormatDirDateTime(ByRef $t)
    ; $t[0]=year, [1]=month, [2]=day, [3]=hour, [4]=minute, [5]=second
    Local $date = StringFormat("%02d/%02d/%04d", $t[1], $t[2], $t[0])
    ...
    Local $time = StringFormat("%02d:%02d %s", $hour, $t[4], $ampm)
    Return $date & "  " & $time & "              0"
EndFunc
...
Func _HandleListClick()
    ...
    ; Use mouse buttons to distinguish
    If _IsPressed("02") Then
        ShellExecute($sDir) ; right-click → open folder
    ElseIf _IsPressed("01") Then
        ; crude double-click detection is tricky here; simplest is single-click open:
