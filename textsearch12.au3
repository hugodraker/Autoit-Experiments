...
While True
    ...
    ; Right-click Browse ? open index file in Notepad
    If $msg = $btnBrowse Then
        Local $ci = GUIGetCursorInfo($hGUI)
        If IsArray($ci) And $ci[2] = 2 Then
...
; -----------------------------
; INDEX FOLDER (native, real timestamps)
; -----------------------------
Func _IndexFolder()
    ...
    While $g_bSearching
        ...
        For $i = 1 To 4000
            ...
            If StringRegExp($sLine, "^\d{2}/\d{2}/\d{4}") Then
                ...
                Local $sFileName = StringRegExpReplace($sLine, "^.*\s", "")
...
; -----------------------------
; HANDLE LIST CLICK (left, right, double)
; -----------------------------
Func _HandleListClick()
    Local $ci = GUIGetCursorInfo($hGUI)
    If Not IsArray($ci) Then Return
    ...
    If TimerDiff($lastClick) < 300 Then
