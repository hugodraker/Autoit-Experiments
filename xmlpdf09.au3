...
Global Const $tagENHMETARECORD = "dword iType;dword nSize;byte Data[1];"
...
; ============================================================
; TABLE RENDERING ENGINE (CSS margins, padding, borders)
; ============================================================
Func _BuildTableStreamCSS(ByRef $aTable, _
    ...
    For $i = 0 To $iRows - 1
        ...
        ; Top border
        $sStream &= $fX0 & " " & $fY & " m " & ($fX0 + $fTotalW * $fScale) & " " & $fY & " l " & $bTop & " w S" & @CRLF

        For $j = 0 To $iCols - 1
            Local $fW = $aColWidths[$j]

            ; Left border
            $sStream &= $fX & " " & $fY & " m " & $fX & " " & ($fY - $fRowH) & " l " & $bLeft & " w S" & @CRLF
        ...
        ; Right border
        $sStream &= $fX & " " & $fY & " m " & $fX & " " & ($fY - $fRowH) & " l " & $bRight & " w S" & @CRLF

        $fY -= $fRowH
    Next

    ; Bottom border
    $sStream &= $fX0 & " " & $fY & " m " & ($fX0 + $fTotalW * $fScale) & " " & $fY & " l " & $bBottom & " w S" & @CRLF
...
; ============================================================
; EMF / WMF VECTOR SUPPORT
; ============================================================
Func _WMF_To_EMF($sWMF)
    Local $aRet = DllCall("gdi32.dll", "handle", "CopyEnhMetaFileW", "wstr", $sWMF, "ptr", 0)
    If @error Or $aRet[0] = 0 Then Return SetError(1, 0, 0)
    Return $aRet[0]
EndFunc

Func _EnumEMF_Callback($hDC, $pTable, $pRecord, $nObj, $lParam)
    Local $tRec = DllStructCreate($tagENHMETARECORD, $pRecord)
    Local $iType = DllStructGetData($tRec, "iType")

    Switch $iType
        Case 0x00000004 ; EMR_MOVETOEX
            Local $tPt = DllStructCreate("long x;long y;", $pRecord + 8)
            Local $x = DllStructGetData($tPt, "x")
            Local $y = DllStructGetData($tPt, "y")
            $g_sPDFVector &= $x & " " & $y & " m" & @CRLF

        Case 0x00000005 ; EMR_LINETO
            Local $tPt = DllStructCreate("long x;long y;", $pRecord + 8)
            Local $x = DllStructGetData($tPt, "x")
            Local $y = DllStructGetData($tPt, "y")
            $g_sPDFVector &= $x & " " & $y & " l S" & @CRLF

        Case 0x00000008 ; EMR_POLYLINE
            Local $nPoints = DllStructGetData(DllStructCreate("dword nPoints;", $pRecord + 8), "nPoints")
            Local $pPoints = $pRecord + 12
            Local $tPoint = DllStructCreate("long x; long y", $pPoints)

            $g_sPDFVector &= DllStructGetData($tPoint, "x") & " " & DllStructGetData($tPoint, "y") & " m" & @CRLF

            For $i = 1 To $nPoints - 1
                $tPoint = DllStructCreate("long x; long y", $pPoints + ($i * 8))
                $g_sPDFVector &= DllStructGetData($tPoint, "x") & " " & DllStructGetData($tPoint, "y") & " l" & @CRLF
            Next
        ...
        Case 0x00000003 ; EMR_POLYGON
            ...
            $g_sPDFVector &= "h S" & @CRLF
    EndSwitch

    Return 1
EndFunc

Func _EMF_To_PDFVector($hEMF, ByRef $fW, ByRef $fH)
    $g_sPDFVector = ""

    Local $tHeader = DllStructCreate("dword iType;dword nSize;" & $tagRECT & "dword nBytes;dword nRecords;word nHandles;word sReserved;dword nDescription;dword offDescription;dword nPalEntries;" & $tagRECT & "dword nBitCount;dword nPlanes;dword nSizeEMF;", 0)
    Local $aHdr = DllCall("gdi32.dll", "dword", "GetEnhMetaFileHeader", "handle", $hEMF, "dword", DllStructGetSize($tHeader), "ptr", DllStructGetPtr($tHeader))
    If Not @error And $aHdr[0] <> 0 Then
        Local $tBounds = DllStructCreate($tagRECT, DllStructGetPtr($tHeader, "Bounds"))
        $fW = DllStructGetData($tBounds, "Right") - DllStructGetData($tBounds, "Left")
        $fH = DllStructGetData($tBounds, "Top") - DllStructGetData($tBounds, "Bottom")
        If $fW < 1 Then $fW = 100
        If $fH < 1 Then $fH = 100
    Else
        $fW = 100
    ...
    If $g_hEnumCB = 0 Then
        $g_hEnumCB = DllCallbackRegister("_EnumEMF_Callback", "int", "ptr;ptr;ptr;int;ptr")
    EndIf

    DllCall("gdi32.dll", "bool", "EnumEnhMetaFile", _
        "handle", $hDC, _
        "handle", $hEMF, _
        "ptr", DllCallbackGetPtr($g_hEnumCB), _
        "ptr", 0, _
        "ptr", 0)
...
Func _PDF_EmbedVectorForm($hFile, $iObjNum, $sVector, $fW, $fH)
    Local $s = _
        $iObjNum & " 0 obj << /Type /XObject /Subtype /Form " & _
        "/BBox [0 0 " & $fW & " " & $fH & "] /Resources << >> /Length " & StringLen($sVector) & " >> stream" & @CRLF & _
        $sVector & "endstream endobj" & @CRLF
    FileWrite($hFile, $s)
EndFunc
...
Func Example_Main()
    Local $hGUI = GUICreate("XML → PDF Generator (Images + Text + CSS Tables + Compression + WMF Vector)", 900, 600)
...
Func _HandleCreatePDF()
    ...
    Local $sOutPDF = $g_sXMLDir & "\output_css_tables_wmf_vector.pdf"
...
; ============================================================
; MAIN PDF ENGINE (TEXT + CSS TABLE + IMAGE/WMF VECTOR)
; ============================================================
Func _PDF_CreateMultiImageWithTextAndTablesCSS($sOutputPDF, ByRef $aImages, ByRef $aTexts, ByRef $aTablesPerPage, $iPageW, $iPageH, $iCompPercent)
    ...
    For $i = 0 To $iCount - 1
        ...
        Local $sImgPath = $aImages[$i]
        Local $sExt = StringLower(StringRight($sImgPath, 4))

        Local $bIsWMF = ($sExt = ".wmf")

        Local $iImgObj = 0
        Local $iVecObj = 0
        Local $fVecW = 0, $fVecH = 0
        Local $sVector = ""

        Local $iScaledW = 0, $iScaledH = 0
        Local $iOrigW = 0, $iOrigH = 0

        ; =====================================================
        ; WMF → EMF → PDF VECTOR PATH
        ; =====================================================
        If $bIsWMF Then
            Local $hEMF = _WMF_To_EMF($sImgPath)
            If @error Or $hEMF = 0 Then
                _GDIPlus_Shutdown()
                FileClose($hOut)
                Return SetError(2, 0, 0)
            EndIf

            $sVector = _EMF_To_PDFVector($hEMF, $fVecW, $fVecH)
            DllCall("gdi32.dll", "bool", "DeleteEnhMetaFile", "handle", $hEMF)

            $iVecObj = _PDF_NewObj()
            _PDF_EmbedVectorForm($hOut, $iVecObj, $sVector, $fVecW, $fVecH)
        ...
        Else
            ; =====================================================
            ; PNG/JPEG → ALWAYS SAVE AS JPEG FOR PDF EMBEDDING
            ; =====================================================
            Local $hImg = _GDIPlus_ImageLoadFromFile($sImgPath)
            If $hImg = 0 Then
                _GDIPlus_Shutdown()
                FileClose($hOut)
            ...
            $iOrigW = _GDIPlus_ImageGetWidth($hImg)
            $iOrigH = _GDIPlus_ImageGetHeight($hImg)

            Local $fScaleComp = $iCompPercent / 100
            $iScaledW = Int($iOrigW * $fScaleComp)
            $iScaledH = Int($iOrigH * $fScaleComp)
        ...
        If $sTableRaw <> "" Then
            ...
            If Not @error And UBound($aInner) >= 2 Then
                ...
                If Not @error Then
                    Local $fTableStartY = $iPageH - $iTopMargin - $iTextHeight
        ...
        ; =====================================================
        ; IMAGE / VECTOR PLACEMENT
        ; =====================================================
        Local $iSpacing = 50
        Local $iBottomMargin = 50

        Local $iTotalTextTableHeight = $iTextHeight + $fTableHeight
        ...
        Local $fImgY = $iPageH - $iTopMargin - $iTotalTextTableHeight - $fDisplayH - $iSpacing
        If $fImgY < $iBottomMargin Then $fImgY = $iBottomMargin

        Local $sImgStream = ""
        If $bIsWMF Then
            $sImgStream = "q " & $fDisplayW & " 0 0 " & $fDisplayH & " 50 " & $fImgY & " cm /Fm0 Do Q" & @CRLF
        Else
            $sImgStream = "q " & $fDisplayW & " 0 0 " & $fDisplayH & " 50 " & $fImgY & " cm /Im0 Do Q" & @CRLF
        EndIf
        ...
        Local $sRes = "/Font << /F1 " & $iFontObj & " 0 R >> "
        If $bIsWMF Then
            $sRes &= "/XObject << /Fm0 " & $iVecObj & " 0 R >>"
        Else
            $sRes &= "/XObject << /Im0 " & $iImgObj & " 0 R >>"
        EndIf
